...
class ContainerCfg(object):
    def __init__(self, name, **kwargs):
        ...
        res_limits = kwargs.get('resources', {}).get('limits', {})
        self.res_limit_cpus = res_limits.get('cpus', None)
        self.res_limit_memory = res_limits.get('memory', None)
...
class ServicesCfg(object):
    def __init__(self, yaml_cfg):
        ...
        for c in self.containers():
            ...
            container['networks'] = [(i, self.networks[i])
                                     for i in container['networks']]
    ...
    def _verify_data(self):
        containers = [container for container in self.services.keys()
                      if len(container) > 5]
        if len(containers) > 0:
            logger.error("The length of the container name > 5:{}".format(
                ' '.join(containers)))
            sys.exit(1)
